...
export default function CompEditNew() {
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        await axios.patch(`http://localhost:3005/api/settlement-news/${id}`, 
            {
                date: date,
    ...
    const getNewById = async () => {
        const response = await axios.get(`http://localhost:3005/api/settlement-news/${id}`);
        const data = await response.data;
        setDate(fromTimestampToDate(data.date));
        setEmployee(data.employeeId);
        setConcept(data.conceptId);
        setQuantity(data.quantity);
        setFactor(data.concept.factor);
        setValue(data.value);
